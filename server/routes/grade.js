...
router.get("/editUser/delete", (req, res) => {
	if (req.session.auth === constData.executivesAuth) {
		...
		if (req.query.target === req.session.username) {
			...
			connection.query("SELECT auth FROM tbluser WHERE username=?", [req.query.target], (error, result) => {
				if (error) {
					console.log(error);
					res.redirect("/grade/editUser");
				} else {
					if (constData.authList[Number(result[0].auth)] != constData.adminAuth) {
						connection.query("DELETE FROM tbluser WHERE username=?", [req.query.target], (error) => {
							if (error) {
								console.log(error);
								res.redirect("/grade/editUser");
							} else {
